...
export default function ScalingStrategy() {
    ...
    // Fetch all scaling strategies
    const getItems = async () => {
        try {
            const res = await axios.get(
                `${import.meta.env.VITE_APP_URL}api/admin/strategy-scaling`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
            setItems(res.data.data || []);
        } catch {
            setError("Failed to load scaling strategies.");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingItem) {
                ...
                await axios.patch(
                    `${import.meta.env.VITE_APP_URL}api/admin/edit-strategy-scaling`,
                    data,
                    config
                );
            } else {
            ...
            setError("Failed to save scaling strategy.");
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axios.delete(
                `${import.meta.env.VITE_APP_URL}api/admin/strategy-scaling/${itemToDelete._id}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
            getItems();
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-700">Scaling Strategy</h2>
                <button
                    onClick={handleAdd}
            ...
            {items?.length == 0 ? (
                <p className="text-center text-gray-500">No scaling strategies found.</p>
            ) : (
