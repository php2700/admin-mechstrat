...
export default function VisionDetail() {
    ...
    const getItems = async () => {
        try {
            const res = await axios.get(
                `${import.meta.env.VITE_APP_URL}api/admin/vision-detail`,
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axios.delete(
                `${import.meta.env.VITE_APP_URL}api/admin/vision-detail/${itemToDelete._id}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
            getItems();
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-700">Vision Strategy</h2>
                <button
                    onClick={handleAdd}
